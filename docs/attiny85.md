...
- **EEPROM** - Non-volatile memory storage (512 bytes)
...
Edit `F_CPU` in `makefiles/attiny85.mk` to match your clock frequency:
...
```bash
# Read current fuses
make MCU=attiny85 read-fuses

# Set fuses for 16MHz internal oscillator
make MCU=attiny85 write-fuses-16mhz

# Set fuses for 8MHz internal oscillator
make MCU=attiny85 write-fuses-8mhz

# Set fuses for 1MHz internal oscillator
make MCU=attiny85 write-fuses-1mhz
```
